$(document).ready(function () {
	...
	var adzunaJobsResultsArray = [];
	var adzunaJobDetailsArray = [];
	...
	function runHomeSearch() {
		...
		$.ajax({
			...
			for (var i = 0; i < 6; i++) {
				var jobsResultsCards = {
					createdDate: response.results[i].created,
					...
					salary: response.results[i].salary_is_predicted,
				};
				var jobDetailsCard = {
					createdDate: response.results[i].created,
					company: response.results[i].company.display_name,
					position: response.results[i].title,
					jobType: response.results[i].category.label,
					country: response.results[i].location.area[0],
					state: response.results[i].location.area[1],
					state: response.results[i].location.area[2],
					city: response.results[i].location.area[3],
					description: response.results[i].description,
					salary: response.results[i].salary_is_predicted,
				};

				console.log(jobsResultsCards);
				console.log(jobDetailsCard);
				adzunaJobsResultsArray.push(jobsResultsCards);
				adzunaJobDetailsArray.push(jobDetailsCard);
			}
		});
	}
